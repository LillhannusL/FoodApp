...
export default function Instructions({ instructions }: InstructionsProps) {
  if (!instructions || instructions.length === 0) {
    return <p className="italic">No instructions avalible...</p>;
  }
  ...
  return (
    <div className="space-y-6 text-light-primary">
        ...
        {allSteps.map((step) => (
          <div key={step.number} className="flex items-center gap-4 sm:gap-8">
            <div className="flex-none">
              <div className="w-8 h-8 rounded-full bg-orange-gradient flex items-center justify-center font-bold shadow-sm">
                {step.number}
              </div>
            </div>

            <div className="flex-1 pt-1">
              <p className="leading-relaxed tracking-wide text-lg text-dark-primary">
